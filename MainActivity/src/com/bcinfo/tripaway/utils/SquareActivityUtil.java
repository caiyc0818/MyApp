...
public class SquareActivityUtil {
	public static void toPersonHomePage(UserInfo userInfo,Context context){
		...
		if (orgRole != null) {
			...
			if ("leader".equals(orgRole.getRoleCode()) || "guide".equals(orgRole.getRoleCode())|| "provider".equals(orgRole.getRoleCode())) {
	...
	public static void toProductHomePage(SquareProductNewInfo productNewInfo2,Context context){
...
//					queryPrivateChatInfo(queueId);
					Intent talkIntent = new Intent(context,
							ChatActivity.class);
...
//				}
	...
	public static void toDetailPage(Feed_Schema pushResource,Context context,boolean extraArg){
		String objectType=pushResource.getFeedType();
		if(!StringUtils.isEmpty(objectType)&&pushResource.getObject()!=null){
			if ("product".equals(objectType)) {
				SquareProductNewInfo productNewInfo2 = (SquareProductNewInfo) pushResource.getObject();
				toProductHomePage(productNewInfo2, context);

			} else if ("commtopic".equals(objectType)) {
//				TopicInfo topicInfo = (TopicInfo) pushResource.getObject();
				...
				intentForTopicProductList.putExtra("themeName", pushResource.getResTitle());
//				intentForTopicProductList.putExtra("description", topicInfo.getDescription());
				context.startActivity(intentForTopicProductList);
			} else if ("commuser".equals(objectType)) {
				UserInfo userInfo2 = (UserInfo) pushResource.getUserInfo();
...
//				startActivity(intentClubFirstPage2);
				SquareActivityUtil.toPersonHomePage(userInfo2, context);
			} else if ("commdestination".equals(objectType)) {
			...
			} else if ("commsofttext".equals(objectType)||"commstory".equals(objectType)) {
				String path = Urls.content_host + pushResource.getObj();
				Intent intent = new Intent(context, ContentActivity.class);
				intent.putExtra("path", path);
				context.startActivity(intent);

			} else if ("commlink".equals(objectType)) {
		String url = (String) pushResource.getObj();
				Intent intent = new Intent(context, ContentActivity.class);
				intent.putExtra("title", "活动");
				intent.putExtra("path", url);
				context.startActivity(intent);
			} else if ("commsubject".equals(objectType)) {
