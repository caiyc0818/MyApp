 ...
 * 
 */
public class PickListAdapter extends BaseAdapter implements OnClickListener {
	...
	public PickListAdapter(List<Map<String, Object>> resultList, Activity mActivity, OperationListener listener) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			convertView = (View) inflator.inflate(R.layout.trip_picked_item, null);
...
//			holder.pickedProductPriceTv.setTypeface(TripawayApplication.tf);// 设置字体
			holder.storeImg = (ImageView) convertView.findViewById(R.id.if_stored);
...
//			holder.pickedProductPeoTv = (TextView) convertView.findViewById(R.id.trip_picked_product_peo_join_tv);// 参与人数
			...
			holder.pickedServicerIv = (ImageView) convertView.findViewById(R.id.trip_picked_product_servicer_iv);
			...
			holder.themeProductLayout1 = (FrameLayout) convertView.findViewById(R.id.theme_product1);
		...
		holder.pv = (TextView) convertView.findViewById(R.id.trip_picked_product_peo_join_tv);	
		...
		if (((String) mapItem.get("pickedItemType")).equals("product")) // 精选 产品
		{
			...
			ProductNewInfo product = (ProductNewInfo) mapItem.get("objectValue");
//			设置原价
...
//			设置浏览人数
			...
			if ((String) (((ProductNewInfo) mapItem.get("objectValue")).getPrice()) != null) {
				holder.pickedProductPriceTv
						.setText("￥" + (String) (((ProductNewInfo) mapItem.get("objectValue")).getPrice()));
				/*
			...
			if (!StringUtils.isEmpty((String) mapItem.get("pickedItemImg"))) {
				ImageLoader.getInstance().displayImage(Urls.imgHost + (String) mapItem.get("pickedItemImg"),
						holder.pickedProductIv, AppConfig.options(R.drawable.default_photo));
			}
			...
			holder.pickedProductDistanceTv
					.setText((String) (((ProductNewInfo) mapItem.get("objectValue")).getDistance()) + "km");
			// add by lij 2015/09/25 start
//			if ("0".equals(((ProductNewInfo) mapItem.get("objectValue")).getMemberJoinCount())) {
//			} else {
//				holder.pickedProductPeoTv
//						.setText(((ProductNewInfo) mapItem.get("objectValue")).getMemberJoinCount() + "人浏览");
//			}
		...
		} else if (((String) mapItem.get("pickedItemType")).equals("user")) // 服务者
		{
			...
			if (!StringUtils.isEmpty((String) mapItem.get("pickedItemImg"))) {
				ImageLoader.getInstance().displayImage(Urls.imgHost + (String) mapItem.get("pickedItemImg"),
						holder.pickedServicerIv, AppConfig.options(R.drawable.ic_launcher));// 服务者
			}

			holder.pickedServicerHeadIconIv.setImageResource(R.drawable.user_defult_photo);
			if (!StringUtils.isEmpty(((UserInfo) mapItem.get("objectValue")).getAvatar())) {
				ImageLoader.getInstance().displayImage(
						...
						holder.pickedServicerHeadIconIv, AppConfig.options(R.drawable.user_defult_photo));// 服务者圆形头像
			}

			holder.pickedServicerNameTv.setText((String) mapItem.get("pickedItemTitle"));// 服务者姓名
			...
			holder.pickedServicerLayoutContainer.setTag(((UserInfo) mapItem.get("objectValue")));

			holder.pickedServicerLayoutContainer.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					if (orgRole != null) {
						if ("admin".equals(orgRole.getRoleCode())) {
							Intent intent = new Intent(mActivity, ClubFirstPageActivity.class);
							intent.putExtra("userInfo", userInfo);
							mActivity.startActivity(intent);
							return;
						}
						if ("leader".equals(orgRole.getRoleCode()) || "guide".equals(orgRole.getRoleCode())) {
							Intent intentForUserInfo = new Intent(mActivity, ClubMebHomepageActivity.class);
					...
					Intent intentForUserInfo = new Intent(mActivity, PersonalInfoNewActivity.class);
					if (profession != null && profession.contains("专业司机")) {
						intentForUserInfo = new Intent(mActivity, ClubMebHomepageActivity.class);
						intentForUserInfo.putExtra("isDriverHomePage", true);
				...
				|| ((String) mapItem.get("pickedItemType")).equals("topic")) // 精选主题
		{
			...
			holder.pickedProductLayoutContainer.setVisibility(View.GONE);
			holder.pickedServicerLayoutContainer.setVisibility(View.GONE);
			holder.pickedProductThemeLayoutContainer.setVisibility(View.VISIBLE);
			...
			holder.themeProductLayout1.setTag(((TopicInfo) mapItem.get("pickedThemeItem1")).getId());
			// if(null != (TopicInfo)mapItem.get("pickedThemeItem2")){
			...
			// });
			// }
			holder.pickedProductThemeNameTv1.setText(((TopicInfo) mapItem.get("pickedThemeItem1")).getTitle());
			...
			// }
			holder.themeProductLayout1.setOnClickListener(new OnClickListener() {
			 ...
			 * "team")){ holder.pickedProductUnit.setVisibility(View.VISIBLE);
			 * holder.pickedProductGroupLayout.setVisibility(View.VISIBLE); }
			 */

		convertView.setTag(R.id.tag_picked_first, holder);
		convertView.setTag(R.id.tag_picked_second, (String) mapItem.get("pickedItemType"));
