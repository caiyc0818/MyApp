 ...
 * @粉丝页面
 * @author hanweipeng
 * @date 2015-6-30 下午2:00:59
 */
public class FansMemberActivity extends BaseActivity implements OnItemClickListener, OnClickListener
{
    ...
    protected void initView()
    {
        ...
        adapter = new FollowAdapter(FansMemberActivity.this, userInfos);
    ...
    private void queryFansMember()
    {
        try
        {
            ...
            HttpUtil.get(Urls.personal_fans_url, params, new JsonHttpResponseHandler()
            {
                ...
                public void onSuccess(int statusCode, Header[] headers, JSONObject response)
                {
                    ...
                    System.out.println("获取粉丝接口=" + response);
                    if (response.optString("code").equals("00000"))
                    {
                        ...
                        if (dataJson != null && !dataJson.equals(""))
                        {
                            ...
                            userInfos.addAll(initFansList(userArray));
                            adapter.notifyDataSetChanged();
                        }
                    }
                    else
                    {
                        ToastUtil.showErrorToast(FansMemberActivity.this, response.optString("msg"));
                    }
                }
     ...
     * 初始化关注或粉丝数据
     */
    protected List<UserInfo> initFansList(JSONArray focusArray)
    {
        ...
        if (focusArray != null)
        {
            for (int i = 0; i < focusArray.length(); i++)
            {
                ...
                {
                    focusUserInfo.setAvatar(userObj.optString("avatar"));
                }
                // 设置
                // 头像
