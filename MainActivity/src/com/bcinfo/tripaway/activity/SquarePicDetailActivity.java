...
public class SquarePicDetailActivity extends BaseActivity implements OnClickListener, IXListViewListener {
	...
	protected void onCreate(Bundle arg0) {
		...
		initTitleLayout("晒图详情");
		statisticsTitle="晒图详情";
	...
	protected void initView() {
		...
		travel_head = (LinearLayout) LayoutInflater.from(this).inflate(R.layout.square_pic_header, null);
		...
		product_servicer_icon_iv.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				ActivityUtil.squareToPersonHomePage(user, SquarePicDetailActivity.this);
	...
	private void addImage() {
		...
		for (int i = 0; i < tripArticle.getPictureList().size(); i++) {
			...
			try {
				ls = new LinearLayout.LayoutParams(dm.widthPixels - gap,
	...
	public void onClick(View v) {
		...
		case R.id.release_button:
			if (!AppInfo.getIsLogin()) {
				Intent it = new Intent(SquarePicDetailActivity.this, LoginActivity.class);
				startActivity(it);
				return;
			}
			Intent intent = new Intent(SquarePicDetailActivity.this, MicroBlogPublishedActivity.class);
		...
		case R.id.product_service_share_button:
			...
			if (tripArticle.getPictureList() != null && tripArticle.getPictureList().size() > 0) {
				ShareSelectDialog shareSelectDialog = new ShareSelectDialog(this,
						tripArticle.getPictureList().get(0).getUrl(), content, tripArticle.getPublisher().getNickname()+"的晒图-远行旅行", tripArticle.getPhotoId(),
						Urls.ShareUrlOfPhoto, tripArticle.getPublisher().getNickname());
				...
				public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
					...
					if (response.optString("code").equals("00000")) {
						...
						if (like == 0) {
							...
							++likes;
							Toast.makeText(SquarePicDetailActivity.this, "点赞 +1", Toast.LENGTH_SHORT).show();
						}
	 ...
	 * test 微游记-照片列表
	 */
	private void testTripStoryUrl(String photoId) {
		HttpUtil.get(Urls.square_pic + photoId, new JsonHttpResponseHandler() {
			@Override
			public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
	...
	private void getTripStoryList(JSONObject response) {
		if ("00000".equals(response.optString("code"))) {
			...
			JSONObject dataObj = response.optJSONObject("data").optJSONObject("tripstory");

			if (dataObj != null && dataObj.length() != 0) {
				tripArticle = JsonUtil.getTripArticle(dataObj);
			} else {
			}
			setData();
		}
	...
	private void setData() {
		...
		if (!StringUtils.isEmpty(userId) && !tripArticle.getPublisher().getUserId().equals(userId)
				&& !StringUtils.isEmpty(tripArticle.getPublisher().getIsFocus())) {
			mFocusTxt.setVisibility(View.VISIBLE);

			if (tripArticle.getPublisher().getIsFocus().equals("yes")) {
				mFocusTxt.setText("取消关注");
				mFocusTxt.setBackgroundResource(R.drawable.shape_gray_rectangle);
				isFocused = true;
		...
		if (StringUtils.isEmpty(tripArticle.getDescription())) {
			tvDescription.setText("");
			 ...
			 richTexts = StringUtils.xmlToRichText(tripArticle.getDescription());
		...
		commentNum.setText(StringUtils.isEmpty(tripArticle.getComments()) ? "0" : tripArticle.getComments());
