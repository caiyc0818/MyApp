...
import com.bcinfo.tripaway.view.ScrollView.PersonalScrollView.onTurnListener;
 ...
 * 
 */
public class PersonalNewScrollView extends ScrollView
{
    ...
    private static final String TAG = "PersonalNewScrollView";
    private int scrolledPositionY;// 当前滚动的y坐标
    private int pointY;// 手指按下去时的y坐标
    ...
    public PersonalNewScrollView(Context context, AttributeSet attrs)
    {
        super(context, attrs);
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev)
    {
        ...
        // int lastPointY = (int) ev.getY();
        // if (scrolledPositionY + getHeight() >= computeVerticalScrollRange()
        // && PersonalInfoNewActivity.firstVisiblePosition == 0 && lastPointY >
        // pointY)
        // {
        //
        // return super.onInterceptTouchEvent(ev);
        // }
        // else if (scrolledPositionY + getHeight() >=
        ...
        case MotionEvent.ACTION_MOVE:// 手指移动
            int lastPointY = (int) ev.getY();
            // if 判断 如果自定义scrollView滚动到最底部 并且gridView的第一个可见的item的position是0
            // 并且手指是向下移动的

            if (scrolledPositionY + getHeight() >= computeVerticalScrollRange()
                    && PersonalInfoNewActivity.firstVisiblePosition == 0 && lastPointY > pointY)
            {

                return super.onInterceptTouchEvent(ev);
            }
            else if (scrolledPositionY + getHeight() >= computeVerticalScrollRange())
            {
                return false;
            }
    ...
    protected void onFinishInflate()
    {
        if (getChildCount() > 0)
        {
            inner = getChildAt(0).findViewById(R.id.footer_personal_container);
        }
    }
